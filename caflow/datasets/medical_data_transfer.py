...
def prepare_training_dataset(output_dir, read_paths, save_names, target_resolution=(96,96,96), split=[0.8, 0.1, 0.1]):
    ...
    for i, index in tqdm(enumerate(permuted_indices)):
        ...
        print('mri shape: ', mri_scan.shape)
        print('pet shape: ', pet_scan.shape)
        ...
        resized_mri_scan = zoom(mri_scan, zoom = calculate_zoom(target_resolution, mri_scan.shape), order=0) #has a weird effect produces values out of range->use order=0
