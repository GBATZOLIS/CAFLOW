...
from caflow.utils.normalisation import WNConv2d
from caflow.utils.processing import concat_elu

class nnflowpp(nn.Module):
    ...
    two layers connected in a residual fashion:
      1. Conv: input -> nonlinearity -> conv3x3 -> nonlinearity -> gate
    ...
    """
    def __init__(self, coupling, in_channels, out_channels=None, num_channels_factor=16, num_blocks=1, drop_prob=0., use_attn=True, aux_channels=None, num_components=32):
        super(nnflowpp, self).__init__()
        self.coupling = coupling
        num_channels = num_channels_factor*in_channels
        if self.coupling == 'MixLog':
