...
class TensorboardConditionalImageSampler(Callback):
    """
    ...
    def on_epoch_end(self, trainer: Trainer, pl_module: LightningModule) -> None:
        val_batch = pl_module.val_dataloader.dataset[-1]
        Y, I = val_batch
        ...
        with torch.no_grad():
            ...
            for sample in range(self.num_samples):
                image = pl_module(Y)
                all_images = torch.cat([all_images, image], dim=0)
            pl_module.train()
        
        all_images = torch.cat([all_images, I], dim=0) #B*(1+self.num_samples+1) concatenated images
            
        grid = torchvision.utils.make_grid(
            tensor=all_images,
            nrow = 1+self.num_samples+1,
